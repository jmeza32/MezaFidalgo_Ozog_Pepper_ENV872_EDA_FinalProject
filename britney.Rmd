---
title: "Marine Stranding Analysis -- Britney's File :)"
author: "Britney Pepper"
date: "4/2/2022"
output: html_document
---

```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)

getwd()

library(tidyverse)
library(tidyr)
library(lubridate)
library(dplyr)

OBIS_data <- read.csv("/Users/britneypepper/Desktop/ENVIRON 872 and L/GitHubRepositories/MezaFidalgo_Ozog_Pepper_ENV872_EDA_FinalProject/Data/Raw/OBIS_seamap_strandings_mystic.csv")

Strandings_Parsed_1 <- OBIS_data %>%
  select(common_name, species_name, date_time, latitude, longitude)

Strandings_Parsed_1$Date <- sapply(strsplit(as.character(Strandings_Parsed_1$date_time), " "), "[", 1)
Strandings_Parsed_1$Time <- sapply(strsplit(as.character(Strandings_Parsed_1$date_time), " "), "[", 2)

Strandings_Parsed_2 <- Strandings_Parsed_1 %>%
  select(common_name, species_name, Date, latitude, longitude)

unique(Strandings_Parsed_2$common_name)

```



...
[continue adding more depth to give background to the analysis here]
...
```
> We reject the null hypothesis that yearly turtle strandings are stationary with a p-value = 0.048.

``` {r turtle monthly time series, message = FALSE, warning = FALSE, include = FALSE}
f_month_turtle <- month(first(Turtle_Stranding_by_date$Month))

Turtle_ts_monthly <- ts(Turtle_Stranding_by_date$Total, 
                  start = c(1990, f_month_turtle), 
                  frequency = 12)
...
```
> We fail to reject the null hypothesis that monthly turtle strandings are stationary with a p-value = 0.107.
